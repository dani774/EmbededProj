...
export function P(props) {
    ...
    let size = wp('4.5%');
    if (props.autoFont){
        fontWeight = getLanguage(props.children) === 'en' ? 'ProductSans-Regular' : 'YekanBakhFaNum-Medium';
    }
    if (props.size){
        size=props.size === 'small' ? wp('4%') : props.size === 'large' ? wp('5%') : props.size;
    }
    return (
        <Text
            style={{...styles.p, fontFamily: fontWeight, fontSize : size, ...props.style,}}
            ellipsizeMode={props.ellipsizeMode || 'tail'}
            numberOfLines={props.numberOfLines} >{props.children}</Text>
    );
}
P.Bold = function Bold(props) {
    ...
    let size = wp('4.5%');
    if (props.autoFont){
        fontWeight = getLanguage(props.children) === 'en' ? 'YekanBakhFaNum-Bold' : 'YekanBakhFaNum-Bold';
    }
    if (props.size){
        size=props.size === 'small' ? wp('4%') : props.size === 'large' ? wp('5%') : props.size;
    }
    return (
        <Text
            style={{...styles.bold, fontFamily: fontWeight, fontSize : size, ...props.style,}}
            ellipsizeMode={props.ellipsizeMode || 'tail'}
            numberOfLines={props.numberOfLines} >{props.children}</Text>
    );
}
P.Regular = function Bold(props) {
    ...
    let size = wp('4.5%');
    if (props.autoFont){
        fontWeight = getLanguage(props.children) === 'en' ? 'ProductSans-Regular' : 'YekanBakhFaNum-Regular' ;
    }
    if (props.size){
        size=props.size === 'small' ? wp('4%') : props.size === 'large' ? wp('5%') : props.size;
    }
...
P.Heavy = function Heavy(props) {
    ...
    let size = wp('4.5%');
    if (props.autoFont){
        fontWeight = getLanguage(props.children) === 'en' ? 'YekanBakhFaNum-Heavy' : 'YekanBakhFaNum-Heavy' ;
    }
    if (props.size){
        size=props.size === 'small' ? wp('4%') : props.size === 'large' ? wp('5%') : props.size;
    }
